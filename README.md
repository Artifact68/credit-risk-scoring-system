...
Проект предназначен для обучения и демонстрации подхода. Решение модели нельзя
использовать как единственное основание для выдачи реального кредита. Для
промышленного применения потребуются проверка качества данных, мониторинг
дрейфа, контроль справедливости модели, аудит признаков и юридическая экспертиза.
